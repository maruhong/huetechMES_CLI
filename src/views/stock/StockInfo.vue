<template>
  ...
  <v-data-table :headers="headers" :items="StockInfo" sort-by="NO"  class="elevation-1">
    <template v-slot:top>
      <template>        
        <v-card class="mx-auto center" dense style="height:80px;padding-top:0px" color="indigo lighten-4">        
          <v-list-item>             
            <v-list-item-title style="max-width:80px;min-width:80px"><p class="font-weight-black">재고 관리</p></v-list-item-title>
                ...
                <v-row dense> 
                  <v-card class="mx-auto align-end" style="height:60px;padding-top:0px;" color="indigo lighten-5">
                   <v-row>                    
                      <v-col md="2" style="height:30px;padding-top:0px">                 
                        <v-select v-model="purchase_gbn" :items="purchase_gbn_options"  label="구분" single ></v-select>
                      </v-col>
                      <v-col md="2" style="height:30px;padding-top:0px">
                      ...
                      <v-col md="2" style="height:38px;padding-top:0px">
                        <v-btn  @click="purchaseDialog=true" class="btn btn-outline-deep-red btn-rounded waves-effect">
                  ...
                  <v-col style="height:38px;padding-top:0px">
                   <!-- 매입 신규 등록 Dialog-->
                    ...
                    <v-dialog v-model="purchaseDialog"  scrollable max-width="800px">
                      ...
                      <v-card >
                        <v-row >
                          <v-col cols="4" class="ms-0" >
                            <v-select  v-model="purchaseGbnList"  :items="purchaseGbnListItems"   item-text="purchaseGbnName"  dense label="매입 구분 선택"  multi-line ></v-select>
                            ...
                            <v-text-field v-model="goodsCnt" label="수량" dense required></v-text-field>
                            <v-text-field v-model="unit" label="단위" dense required></v-text-field>
                          </v-col>
                          <v-col cols="4" class="ms-0" >
                            
                            <v-text-field v-model="unitPrice" label="매입단가" dense required></v-text-field> 
                            <v-text-field v-model="amountPrice" label="매입총금액" dense required></v-text-field> 
                            <v-text-field v-model="prepaymentPrice" label="선수금" dense required></v-text-field>
                            <v-text-field v-model="paymentPrice" label="지불금액" dense required></v-text-field>
                            <v-text-field v-model="paymentCnt" label="회차" dense required></v-text-field>
                            ...
                            <v-menu ref="paidDateMenu" v-model="paidDateMenu" :return-value.sync="paidDate" :close-on-content-click="false" 
                              ...
                              <v-date-picker v-model="paidDate" no-title dense scrollable>
                                <div class="flex-grow-1"></div>
                                <v-btn text color="primary" @click="opaidDateMenu = false">Cancel</v-btn>
                                <v-btn text color="primary" @click="$refs.paidDateMenu.save(paidDate)">OK</v-btn>
                              </v-date-picker> 
                            </v-menu>  
                            </v-col>
                            <v-text-field v-model="paymentAmountPrice" label="지불총금액" dense required></v-text-field>
                          </v-col>
                            <v-col cols="4" class="ms-0" >                            
                            <v-select  v-model="paymentMethodList"  :items="paymentMethodListItems"   item-text="paymentMethodName"   label="지불방법"  multi-line dense></v-select>
                            
                            <v-text-field v-model="text" label="잔액" dense required></v-text-field>
                            <v-col cols="12" md="20" style="height:70px;padding-top:0px">
                            <v-menu ref="enterDateMenu" v-model="enterDateMenu" :return-value.sync="enterDate" :close-on-content-click="false" 
                              transition="scale-transition" offset-y min-width="290px">
                              <template v-slot:activator="{ on }">
                                <v-text-field
                                  label="입고일"
                              ...
                              <v-date-picker v-model="enterDate" no-title dense scrollable>
                                <div class="flex-grow-1"></div>
                                <v-btn text color="primary" @click="enterDateMenu = false">Cancel</v-btn>
                                <v-btn text color="primary" @click="$refs.enterDateMenu.save(enterDate)">OK</v-btn>
                              </v-date-picker> 
                            </v-menu>  
                            </v-col>
                            <v-select  v-model="finishPaymentList"  :items="finishPaymentListItems"   item-text="finishPaymentName"  dense label="지불완료여부"  multi-line ></v-select>
                            </v-col>
                        </v-row>
                        <v-divider></v-divider>
                        <v-card-actions>
