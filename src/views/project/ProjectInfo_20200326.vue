...
<v-section>  
  <v-container fluid grid-list-md> 
    <v-layout sx12 md4 lg3 xl2 row wrap fill-height> 
      <v-flex dense align-end > 
        <v-card height="100px"  class="d-flex flex-center"  dense tile>
          <v-select ma-3 outlined dense v-model="s_projectId"  :items="projectIdItems" item-text="projectId"  label="도면번호 선택"  ></v-select> 
          <v-menu  ma-3 dense v-model="orderStartDateMenu"  
          ...
          <v-menu ma-3 dense v-model="orderEndDateMenu"  
                  :close-on-content-click="true"  
          ...
          <v-text-field ma-3 outlined dense v-model="s_modelName" label="모델명 입력" multi-line ></v-text-field>                 
          <v-text-field ma-3 outlined dense v-model="s_productName" label="제품명 입력" multi-line ></v-text-field>               
          <v-icon  >mdi-file-search</v-icon>
          <v-spacer></v-spacer>
          <v-icon @click="pop_projectIdRegistered=true">mdi-comment-plus</v-icon>
         </v-card> 
      </v-flex>
    </v-layout>      
   </v-container>
